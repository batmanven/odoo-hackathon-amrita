...
export default async function TripViewPage({ params }: { params: { id: string } }) {
  ...
  const actualExpenses = trip.stops.reduce((sum, stop) =>
    sum + stop.activities.reduce((aSum, act) => aSum + (act.cost || 0), 0), 0
  )
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 pb-20">
      ...
      {/* View Header with Filters - Excalidraw Style */}
      <div className="max-w-6xl mx-auto px-6 mt-10 space-y-12">
        ...
        <section className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex flex-wrap gap-3">
            <Button variant="outline" className="h-14 px-6 rounded-2xl border-none bg-white shadow-sm hover:bg-gray-50 text-gray-700 font-bold cursor-pointer">
              Group by
            </Button>
            <Button variant="outline" className="h-14 px-6 rounded-2xl border-none bg-white shadow-sm hover:bg-gray-50 text-gray-700 font-bold cursor-pointer">
        ...
        {/* Timeline Grid - Excalidraw Style */}
